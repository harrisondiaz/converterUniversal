...
class YtDlpService {
  ...
  resolveBinary() {
    ...
    for (const systemBin of ['/usr/local/bin/yt-dlp', '/usr/bin/yt-dlp']) {
      if (fs.existsSync(systemBin)) return systemBin;
    }
  ...
  getInstallHint() {
    if (process.env.VERCEL) {
      return 'Vercel cannot run yt-dlp reliably. Deploy with Railway or Render (see README).';
    }
